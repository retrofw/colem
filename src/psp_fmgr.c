...
psp_fmgr_getExtId(const char *szFilePath) 
{
  ...
  if((pszExt = strrchr(szFilePath, '.'))) {
    ...
    if (!strcasecmp(pszExt, ".usb")) {
      return FMGR_FORMAT_USB;
    } else
  ...
  while(nfiles<GP2X_FMGR_MAX_ENTRY){
    ...
    if ( (format == user_file_format               ) || 
         ( (format == FMGR_FORMAT_ZIP            ) && 
           ...
           (user_file_format != FMGR_FORMAT_USB  ) &&
...
static char user_filedir_usb[GP2X_FMGR_MAX_NAME];
...
psp_fmgr_menu_load(int format, const char *_user_filename)
{
  ...
  if (first) {
    ...
    strcpy(user_filedir_usb, user_filedir_kbd);
    ...
    strcat(user_filedir_usb, "/usb/");
    strcat(user_filedir_save, "/save/");
    user_filedir_rom = CV.rom_directory;
  }
  ...
  if (format == FMGR_FORMAT_USB) user_filedir = user_filedir_usb;
  else
  ...
  if (1 || !strcmp(user_filename, "") || psp_file_request(user_filename, user_filedir)) {
    ...
    else 
    {
      ...
      else 
      {
        ...
        if (file_format == FMGR_FORMAT_USB) /* Usb */  error = psp_joy_load_usb_config(user_filename);
        else
