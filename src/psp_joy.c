...
psp_joy_load_settings_file(FILE *JoyFile)
{
  ...
  while (fgets(Buffer,512,JoyFile) != (char *)0) {
    ...
    *Scan = '\0';
    ...
    if (!strcasecmp(Buffer,"player_joy_0")) CV.player_joy_0 = Value;
    else
    if (!strcasecmp(Buffer,"player_joy_1")) CV.player_joy_1 = Value;
    else
    if (!strcasecmp(Buffer,"player_joy_2")) CV.player_joy_2 = Value;
    else
...
psp_joy_save_settings(char *joy_filename)
{
  ...
  if (JoyFile != (FILE*)0) {
    ...
    fprintf( JoyFile, "player_joy_0=%d\n"         , CV.player_joy_0);
    fprintf( JoyFile, "player_joy_1=%d\n"         , CV.player_joy_1);
    fprintf( JoyFile, "player_joy_2=%d\n"         , CV.player_joy_2);
