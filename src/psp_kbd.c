...
# define KBD_MIN_AUTOFIRE_TIME   1000000
 ...
 static gp2xCtrlData   loc_button_data[GP2X_MAX_JOY];
 ...
 static char           loc_button_press[GP2X_MAX_JOY][ KBD_MAX_BUTTONS ]; 
 static char           loc_button_release[GP2X_MAX_JOY][ KBD_MAX_BUTTONS ]; 
...
int
cv_key_event(int cv_idx, int joy_id, int press)
{
  int joy;
  if (joy_id == 0) joy = CV.player_joy_0;
  else
  if (joy_id == 1) joy = CV.player_joy_1;
  else             joy = CV.player_joy_2;
...
psp_kbd_enter_danzeff()
{
  ...
  if (danzeff_cv_pending) 
  {
    if ((c.TimeStamp - loc_last_event_time) > KBD_MIN_PENDING_TIME) {
      ...
      cv_key_event(danzeff_cv_key, 0, 0);
    }
  ...
  if (danzeff_key > DANZEFF_START) {
    ...
    if (cv_key != -1) {
      ...
      cv_key_event(danzeff_cv_key, 0, 1);
    }
...
int
cv_decode_key(int psp_b, int joy_id, int button_pressed)
{
  int wake = 0;
    ...
    if (psp_kbd_mapping[psp_b] >= 0) {
      ...
      if (button_pressed) {
        ...
        if (kbd_ltrigger_mapping_active) {
          ...
          cv_key_event(psp_kbd_presses[psp_b], joy_id, button_pressed);
        } else
...
kbd_reset_button_status(void)
{
  ...
  for (j = 0; j < GP2X_MAX_JOY; j++) {
    for (b = 0; b < KBD_MAX_BUTTONS; b++) {
      loc_button_press[j][b]   = 0;
      loc_button_release[j][b] = 0;
    }
  }
  psp_init_keyboard();
  return 0;
...
kbd_scan_keyboard(void)
{
  ...
  if (CV.cv_auto_fire && (joy_id == 0)) {
    ...
    if ((delta_stamp < 0) || 
      ...
      cv_key_event(CV_JOY_FIRE1, joy_id, CV.cv_auto_fire_pressed);
  ...
  for (b = 0; b < KBD_MAX_BUTTONS; b++) 
  {
    if (c.Buttons & loc_button_mask[b]) {
      if (!(loc_button_data[joy_id].Buttons & loc_button_mask[b])) {
        loc_button_press[joy_id][b] = 1;
        event = 1;
      }
    } else {
      if (loc_button_data[joy_id].Buttons & loc_button_mask[b]) {
        loc_button_release[joy_id][b] = 1;
        loc_button_press[joy_id][b] = 0;
        event = 1;
      }
    }
  }
  memcpy(&loc_button_data[joy_id],&c,sizeof(gp2xCtrlData));
...
psp_update_keys(void)
{
  ...
  static int release_pending[GP2X_MAX_JOY] = { 0, 0, 0 };
  ...
  if (first_time) {
    ...
    for (j = 0; j < GP2X_MAX_JOY; j++) {
      ...
      gp2xCtrlPeekBufferPositive(&loc_button_data[j], 1);
      loc_button_data[j].Buttons = 0;
  ...
  for (j = 0; j < GP2X_MAX_JOY; j++) {
    if (release_pending[j])
    {
      release_pending[j] = 0;
      for (b = 0; b < KBD_MAX_BUTTONS; b++) {
        if (loc_button_release[j][b]) {
          loc_button_release[j][b] = 0;
          loc_button_press[j][b] = 0;
          cv_decode_key(b, j, 0);
        }
